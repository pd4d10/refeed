...
export async function fetchList() {
  ...
  for (const subscription of subscriptions) {
    const categories = subscription.categories.filter(c =>
      /\/label\/.*$/.test(c.id)
    )
  ...
  return {
    ...
    items: items.map(item => ({
      ...
      starred: item.categories.some(c =>
        /\/state\/com.google\/starred$/.test(c)
      ),
    })),
  }
}
...
export async function markAsRead(id) {
  const res = await fetchInoreader(
    `/edit-tag?i=${id}&a=user/-/state/com.google/read`,
    {
      // headers: {
